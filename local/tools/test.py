...
def repeat_eval_ckpt(model, test_loader, args, eval_output_dir, logger, ckpt_dir):
    ...
    while True:
        ...
        zilong_model = False
        print(cfg.MODEL.TEST)
        while(1):
            pass
        if 'ZILONG_MODEL' in cf.MODEL.TEST:
            zilong_model =  cfg.MODEL.TEST.ZILONG_MODEL
            
        
        model.load_params_from_file(filename=cur_ckpt, logger=logger, zilong_model=zilong_model)
        model.cuda()
