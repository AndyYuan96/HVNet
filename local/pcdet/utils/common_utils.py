...
def mask_points_by_range(points, limit_range):
    mask = (points[:, 0] >= limit_range[0]) & (points[:, 0] <= limit_range[3] - 0.01) \
           & (points[:, 1] >= limit_range[1]) & (points[:, 1] <= limit_range[4] - 0.01) \
           & (points[:, 2] >= limit_range[2]) & (points[:, 2] <= limit_range[5] - 0.01)
    points = points[mask]
    return points
