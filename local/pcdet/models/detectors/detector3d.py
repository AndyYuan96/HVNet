...
class Detector3D(nn.Module):
    ...
    def build_networks(self, model_cfg):
        ...
        if cfg.MODEL.NAME =='HVNet' or cfg.MODEL.RPN.RPN_HEAD.NAME == 'FPNHead':
    ...
    def predict_boxes(self, rpn_ret_dict, rcnn_ret_dict, input_dict):
        ...
        if rcnn_ret_dict is None:
            if isinstance(rpn_ret_dict['anchors'], dict): #cfg.MODEL.NAME == 'HVNet' or cfg.MODEL.RPN.RPN_HEAD.NAME == 'FPNHead':
                batch_box_preds = []
                batch_cls_preds = []
                for cur_class in cfg.CLASS_NAMES:
                    anchors = rpn_ret_dict['anchors'][cur_class]
                    batch_anchors = anchors.view(1, -1, anchors.shape[-1]).repeat(batch_size, 1, 1)
                    num_anchors = batch_anchors.shape[1]
                    cls_preds = rpn_ret_dict[cur_class][0].view(batch_size, num_anchors, -1).float()
                    box_preds = rpn_ret_dict[cur_class][1].view(batch_size, num_anchors, -1).float()
                    batch_cls_preds.append(cls_preds)

                    batch_box_pred = self.rpn_head.box_coder.decode_with_head_direction_torch(
                        box_preds=box_preds,
                        anchors=batch_anchors,
                        dir_cls_preds=rpn_ret_dict[cur_class][2] if len(rpn_ret_dict[cur_class]) > 2 else None, #rpn_ret_dict.get('rpn_dir_cls_preds', None),
                    ...
                    batch_box_preds.append(batch_box_pred)
    ...
    def post_processing(self, batch_cls_preds, batch_box_preds, rcnn_ret_dict, input_dict):
        ...
        if isinstance(batch_cls_preds, list):
            # multi-head is on different feature map
        ...
        if isinstance(batch_cls_preds, list):
            # [class1_batch, class2_batch, class3_batch]
            for index in batch_index:
                ...
                if cfg.MODEL.TEST.MULTI_CLASSES_NMS:
                    for i, cur_class in enumerate(cfg.CLASS_NAMES):
                        cur_cls_preds = batch_cls_preds[i][index]
                        cur_box_preds = batch_box_preds[i][index]
                        assert(len(cur_cls_preds.shape) == 2)
                        assert(len(cur_box_preds.shape) == 2)

                        if not cfg.MODEL.RPN.RPN_HEAD.ARGS['encode_background_as_zeros']:
                            cur_cls_preds = cur_cls_preds[..., 1:]
                        # for sigle classification
                        if cur_cls_preds.shape[1] == 1 and len(cfg.CLASS_NAMES) > 1: 
                            rank_scores = cur_cls_preds.new_full((cur_cls_preds.shape[0], len(cfg.CLASS_NAMES)), torch.finfo(input.dtype).min)
                            rank_scores[:,i] = cur_cls_preds
                        else:
                            rank_scores = cur_cls_preds
                        
                        normalized_scores = torch.sigmoid(rank_scores)

                        rank_scores_list.append(rank_scores)
                        normalized_scores_list.append(normalized_scores)
                        box_preds_list.append(cur_box_preds)
                    # now finish one sample 
                    rank_scores_final = torch.cat(rank_scores_list, dim=0)
                    normalized_scores_final = torch.cat(normalized_scores_list, dim=0)
                    box_preds_final = torch.cat(box_preds_list, dim=0)

                    selected, final_labels = self.multi_classes_nms(
                        rank_scores=rank_scores_final,
                        normalized_scores=normalized_scores_final,
                        box_preds=box_preds_final,
                        score_thresh=cfg.MODEL.TEST.SCORE_THRESH,
                        nms_thresh=cfg.MODEL.TEST.NMS_THRESH,
                        nms_type=cfg.MODEL.TEST.NMS_TYPE
                    )
                    final_scores = rank_scores_final.new_empty(selected.shape[0])
                    if cfg.MODEL.TEST.USE_RAW_SCORE:
                        for m in range(selected.shape[0]):
                            final_scores[m] = rank_scores_final[selected[m], final_labels[m] - 1]
                    else:
                        for m in range(selected.shape[0]):
                            final_scores[m] = normalized_scores_final[selected[m], final_labels[m] - 1]
                    
                    final_boxes = box_preds_final[selected]
                else:
                    for i, cur_class in enumerate(cfg.CLASS_NAMES):
                        # which class 's which sample
                        cur_cls_preds = batch_cls_preds[i][index]
                        cur_box_preds = batch_box_preds[i][index]
                        assert(len(cur_cls_preds.shape) == 2)
                        assert(len(cur_box_preds.shape) == 2)
                        ...
                        if not cfg.MODEL.RPN.RPN_HEAD.ARGS['encode_background_as_zeros']:
                            cur_cls_preds = cur_cls_preds[..., 1:]
                        if cur_cls_preds.shape[1] == 1:
                            rank_scores = cur_cls_preds.squeeze(1)
                            class_labels = i * torch.ones_like(rank_scores, dtype=torch.long)
                            normalized_scores = torch.sigmoid(rank_scores)
                        else:
                            rank_scores, class_labels = torch.max(cur_cls_preds, dim=-1)
                            normalized_scores = torch.sigmoid(rank_scores)
                    ...
                    rank_scores_sample = torch.cat(rank_scores_list)
                    normalized_scores_sample = torch.cat(normalized_scores_list)
                    class_labels_sample = torch.cat(class_labels_list)
                    box_preds_sample = torch.cat(box_preds_list, dim=0)
                 
                    selected = self.class_agnostic_nms(
                        rank_scores=rank_scores_sample,
                        normalized_scores=normalized_scores_sample,
                        box_preds=box_preds_sample,
                        score_thresh=cfg.MODEL.TEST.SCORE_THRESH,
                        nms_thresh=cfg.MODEL.TEST.NMS_THRESH,
                        nms_type=cfg.MODEL.TEST.NMS_TYPE
                    )

                    final_labels = class_labels_sample[selected]
                    final_scores = rank_scores_sample[selected] if cfg.MODEL.TEST.USE_RAW_SCORE else normalized_scores_sample[selected]
                    final_boxes = box_preds_sample[selected]
                
                record_dict = {
                'boxes': final_boxes,
                'scores': final_scores,
                'labels': final_labels}
                if 'DEBUG_CONFIG' in cfg and cfg['DEBUG_CONFIG']['ENABLE'] == True:
                    print(final_scores.sigmoid())
                    print(final_labels)
                ...
                global steps
                if 'DEBUG_CONFIG' in cfg and cfg['DEBUG_CONFIG']['ENABLE'] == True:
                    pc_path = cfg['DEBUG_CONFIG']['PC_PATH']
                    cur_pc_path = os.path.join(pc_path, "{}.bin".format(steps))
                    gt_boxes = input_dict['gt_boxes'][0, :, :7]
                    gt_boxes = gt_boxes.cpu().numpy()
                    tt = gt_boxes[:,3].copy()
                    gt_boxes[:,3] = gt_boxes[:,4]
                    gt_boxes[:,4] = tt
                    gt_boxes[:,2] += gt_boxes[:,5]/2
                    gt_boxes[:,6] = -np.pi * 0.5 - gt_boxes[:,6]

                    box3d_lidar = cur_pred_dict['box3d_lidar']
                    tt = box3d_lidar[:,3].copy()
                    box3d_lidar[:,3] = box3d_lidar[:,4]
                    box3d_lidar[:,4] = tt
                    box3d_lidar[:,2] += box3d_lidar[:,5]/2
                    box3d_lidar[:,6] = -np.pi * 0.5 - box3d_lidar[:,6]
                    visual_utils.display_pcd(cur_pc_path, weishu=4,gt=gt_boxes,detection=box3d_lidar)
                    steps += 1
    ...
    def multi_classes_nms(rank_scores, normalized_scores, box_preds, score_thresh, nms_thresh, nms_type='nms_gpu'):
        ...
        selected_labels = torch.cat(selected_labels, dim=0) if selected_labels.__len__() > 0 else []
