...
class AnchorHeadMultiFeature(nn.Module):
    ...
    def assign_targets(self, gt_boxes):
        ...
        #     cur_pc_path = os.path.join(pc_path, "{}.bin".format(debug_counts))

        #     anchors = []
        #     box_targets = []
        #     label_vals = targets_dict['labels']
        #     bbox_target_vals = targets_dict['bbox_targets']
        #     for name, label, box_target in zip(cfg.CLASS_NAMES, label_vals, bbox_target_vals):
        #         if name != 'Car':
        #             continue
        #         negative_one_dim_anchor = self.anchor_cache['anchors_dict'][name]['anchors'].shape[-1]
        #         negative_one_dim_target = box_target.shape[-1]

        #         cur_anchor = self.anchor_cache['anchors_dict'][name]['anchors'].reshape(-1,negative_one_dim_anchor)
        #         label = label.squeeze(0).unsqueeze(1)
        #         label = label.cpu().numpy()
        #         box_target = box_target.squeeze(0).cpu().numpy()

        #         label_mask1 = label.reshape(-1,1) * np.ones_like(cur_anchor)
        #         cur_anchor = cur_anchor[label_mask1 > 0]
        #         cur_anchor = cur_anchor.reshape(-1,negative_one_dim_anchor)
        #         anchors.append(cur_anchor)

        #         label_mask2 = label.reshape(-1,1) * np.ones_like(box_target)
        #         cur_box_target = box_target[label_mask2 > 0]
        #         cur_box_target = cur_box_target.reshape(-1, negative_one_dim_target)
        #         box_targets.append(cur_box_target)
            
        #     gt_boxes = gt_boxes.squeeze(0)
        #     anchor = np.vstack(anchors)
        #     box_target = np.vstack(box_targets)

        #     anchor_target_cfg = cfg.MODEL.RPN.RPN_HEAD.TARGET_CONFIG
        #     cur_coder = getattr(box_coder_utils, anchor_target_cfg.BOX_CODER)()
        #     decoded_objects = cur_coder.decode_np(box_target, anchor)

        #     # for i in range(decoded_objects.shape[0]):
        #     #     print(i)
        #     #     print("gt: ")
        #     #     print(decoded_objects[i])
        #     #     print("encoded: ")
        #     #     print(box_target[i])
        #     #     print("anchor: ")
        #     #     print(anchor[i])

        #     tt = decoded_objects[:,3].copy()
        #     decoded_objects[:,3] = decoded_objects[:,4]
        #     decoded_objects[:,4] = tt
        #     decoded_objects[:,2] += decoded_objects[:,5]/2
        #     decoded_objects[:,6] = np.pi * 0.5 - decoded_objects[:,6]
        ...
        #     visual_utils.display_pcd(cur_pc_path, weishu=4,gt=gt_boxes,assigned_anchor=anchor)#, assigned_anchor=anchor)
        #     input()
        #     debug_counts += 1
...
class HVHead(AnchorHeadMultiFeature):
    ...
    def forward(self, scale_features, **kwargs):
        ...
        # for i in [2]:
        #     for j in range(1):
        #         for m in range(output_features[i][j].shape[0]):
        #             feature = output_features[i]
        #             feature = feature[j]
        ...
        #             plt.imsave('/media/ovo/file3/detection/PCDet/debug/pics/output_{}_{}_{}.jpg'.format(j, i, m),feature)
        ...
        if 'Pedestrian' in cfg.CLASS_NAMES:
            ...
            # feature = ret_dict['Pedestrian'][0][0,:,:,0].sigmoid()
            # feature = feature.cpu().detach().numpy()# * 255
            # plt.imsave('/media/ovo/file3/detection/PCDet/debug/pics/Pedestrian.jpg',feature)
        ...
        if 'Cyclist' in cfg.CLASS_NAMES: #cfg.CLASS_NAMES:
            ...
            if self._use_direction_classifier:
                ret_dict['Cyclist'].append(self.output_blocks[-2][2](output_features[-2]).permute(0,2,3,1).contiguous())
            # feature = ret_dict['Cyclist'][0][0,:,:,1].sigmoid()
            # feature = feature.cpu().detach().numpy()# * 255
            # plt.imsave('/media/ovo/file3/detection/PCDet/debug/pics/Cyclist.jpg',feature)
        ...
        if 'Car' in cfg.CLASS_NAMES:
            ...
            if self._use_direction_classifier:
                ret_dict['Car'].append(self.output_blocks[-1][2](output_features[-1]).permute(0,2,3,1).contiguous())
            # feature = ret_dict['Car'][0][0,:,:,2].sigmoid()
            # feature = feature.cpu().detach().numpy()# * 255
            # plt.imsave('/media/ovo/file3/detection/PCDet/debug/pics/Car.jpg',feature)
