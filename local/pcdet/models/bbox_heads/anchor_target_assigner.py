...
class TargetAssigner(object):
    ...
    def generate_anchors(self, feature_map_size=None, use_multi_head=False):
        ...
        for anchor_generator, match_thresh, unmatch_thresh in zip(self.anchor_generators,
            ...
            if use_multi_head:
                ...
                anchors = anchors.reshape([*anchors.shape[:3], -1, anchors.shape[-1]]) # (1, H, W, 2#, code_size)
                ndim = len(anchor_generator.feature_map_size) #(batch, h, w)
                anchors = anchors.transpose(ndim, *range(0, ndim), ndim + 1)  # (2#, 1, H, W, code_size)
                anchors = anchors.reshape(-1, anchors.shape[-1])
            else:
                anchors = anchor_generator.generate(feature_map_size) # (1, 248, 216, 1(1 type anchor), 2(two angle), 7)
    ...
    def generate_anchors_dict(self, feature_map_size, use_multi_head=False, zl=False):
        anchors_list = []
        matched_thresholds = [a.match_threshold for a in self.anchor_generators]
        unmatched_thresholds = [a.unmatch_threshold for a in self.anchor_generators]
        match_list, unmatch_list = [], []
        ...
        for anchor_generator, match_thresh, unmatch_thresh in zip(self.anchor_generators,
                                                                  matched_thresholds, unmatched_thresholds):
            if use_multi_head:
                anchors = anchor_generator.generate(anchor_generator.feature_map_size) # (1, H, W, 1(one type anchor), 2 , code_size)
                anchors = anchors.reshape([*anchors.shape[:3], -1, anchors.shape[-1]]) # (1, H, W, 2, code_size)
                if not zl:
                    ndim = len(feature_map_size)
                    anchors = anchors.transpose(ndim, *range(0, ndim), ndim + 1) # (2(types * angles), 1, H, W, code_size)
            else:
            ...
            anchors_dict[class_name]['anchors'] = anchors  #(batch, h, w, c, code_size)
            anchors_dict[class_name]['matched_thresholds'] = match_list[-1] #(batch * h * w * c)
    ...
    #     #[x, y, xdim, ydim, rad]
    #     # to [x1,y1,x2,y2,rad]
    #     # convert to all lidar coordinate
    #     box1 = boxes1.copy()
    #     box2 = boxes2.copy()
    #     box1 = torch.from_numpy(box1).cuda()
    #     box1 = torch.from_numpy(box1).cuda()
        
    #     xyxy_boxes1 = boxes1.new_full(boxes1.shape,0.0)
    #     xyxy_boxes1[:,0] = boxes1[:,0] - boxes1[:,2] * 0.5
    #     xyxy_boxes1[:,1] = boxes1[:,1] - boxes1[:,3] * 0.5
    #     xyxy_boxes1[:,2] = boxes1[:,0] + boxes1[:,2] * 0.5
    #     xyxy_boxes1[:,3] = boxes1[:,1] + boxes1[:,3] * 0.5
    #     xyxy_boxes1[:,4] = boxes1[:,4]

    #     xyxy_boxes2 = boxes2.new_full(boxes2.shape,0.0)
    #     xyxy_boxes2[:,0] = boxes2[:,0] - boxes2[:,2] * 0.5
    #     xyxy_boxes2[:,1] = boxes2[:,1] - boxes2[:,3] * 0.5
    #     xyxy_boxes2[:,2] = boxes2[:,0] + boxes2[:,2] * 0.5
    #     xyxy_boxes2[:,3] = boxes2[:,1] + boxes2[:,3] * 0.5
    #     xyxy_boxes2[:,4] = boxes2[:,4]

    #     ret = iou3d_nms_utils.boxes_iou_bev(xyxy_boxes1, xyxy_boxes2)
    #     ret = ret.cpu()
    #     ret = ret.numpy()
    ...
    def rotated_iou_similarity(boxes1, boxes2):
        ...
        # to [x1,y1,x2,y2,rad]
        # convert to all lidar coordinate
        # boxes1 is anchor, anchor shouldn't change angle

        box1 = boxes1.copy()
        box2 = boxes2.copy()
        box1 = torch.from_numpy(box1).cuda()
        box2 = torch.from_numpy(box2).cuda()
        
        xyxy_boxes1 = box1.new_full(box1.shape,0.0)

        xyxy_boxes1[:,0] = box1[:,0] - box1[:,2] * 0.5
        xyxy_boxes1[:,1] = box1[:,1] - box1[:,3] * 0.5
        xyxy_boxes1[:,2] = box1[:,0] + box1[:,2] * 0.5
        xyxy_boxes1[:,3] = box1[:,1] + box1[:,3] * 0.5
        xyxy_boxes1[:,4] = box1[:,4]

        xyxy_boxes2 = box2.new_full(boxes2.shape,0.0)
        xyxy_boxes2[:,0] = box2[:,0] - box2[:,2] * 0.5
        xyxy_boxes2[:,1] = box2[:,1] - box2[:,3] * 0.5
        xyxy_boxes2[:,2] = box2[:,0] + box2[:,2] * 0.5
        xyxy_boxes2[:,3] = box2[:,1] + box2[:,3] * 0.5

        if 'LIDAR_ASSIGN' in cfg.MODEL.RPN.RPN_HEAD.TARGET_CONFIG and cfg.MODEL.RPN.RPN_HEAD.TARGET_CONFIG['LIDAR_ASSIGN']:
            xyxy_boxes2[:,4] = np.pi * 0.5 - box2[:,4]
    ...
    def assign_v2(self, anchors_dict, gt_boxes, anchors_mask=None, gt_classes=None, gt_names=None, zl=False, multi_class_predict=False):
        ...
        for class_name, anchor_dict in anchors_dict.items():
            ...
            targets = self.create_target_np(
                ...
                gt_classes=gt_classes[mask] if multi_class_predict else None,
    ...
    def create_target_np(self, all_anchors,
        ...
        if len(gt_boxes) > 0 and anchors.shape[0] > 0:
            ...
            #移除掉与每个anchor都不想交的gt,规则是与改gt相交的anchor的iou的最大值为0
            ...
            # 找到与某个gt满足最大iou的anchor的序号，某个anchor可能出现多次
            # 返回的是比较之后满足条件的坐标，这里anchor_by_gt : (102136,13), gt_to_anchor_max(13,)
            # 比较之后返回两个array, 代表满足条件的数据的坐标，第一维度即为x的坐标，即102136上的坐标,坐标即代表某个anchor与gt的iou满足是这个gt对应的iou的最大值
            anchors_with_max_overlap = np.where(
                anchor_by_gt_overlap == gt_to_anchor_max)[0]
            
            # Fg label: for each gt use anchors with highest overlap
            # (including ties)
            # 这里不是很好，anchors_with_max_overlap：对某个与gt有最大iou的anchor的id,想要通过这个id找到这个gt, 但是用的确实对这个anchor来说，与他的iou最大的gt, 这个gt不一定是前面的gt，你默认了若anchor与某个gt的iou最大，那么这个gt与所有的anchor的iou中，与这个anchor的iou最大.
        ...
        # print(fg_inds)
        # print("fg overlap: ")
        # print(anchor_by_gt_overlap[fg_inds])
