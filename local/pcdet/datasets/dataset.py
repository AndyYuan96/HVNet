...
class DatasetTemplate(torch_data.Dataset):
    ...
    def prepare_data(self, input_dict, has_label=True):
        ...
        if self.training:
            ...
            if noise_global_scene.ENABLED:
                ...
                if "GLOBAL_LOC_NORM_STD" in noise_global_scene.keys():
                    gt_boxes, points = augmentation_utils.global_translate(gt_boxes, points, noise_global_scene.GLOBAL_LOC_NORM_STD)
        ...
        if cfg.MODEL.NAME == 'MVF':
            ...
            example.update({
                'bev_points' : points,
                'voxel_local_coordinates':[],
                'bev_mapping_pvs' : [],
                'bev_mapping_vfs' : [],
                'calib' : input_dict['calib']
            })

            for j in range(len(self.hv_features)):
                # for each point , caculate it's voxel according to 
                voxel_generator_cfg = cfg.DATA_CONFIG.VOXEL_GENERATOR
                bev_local_coordinate = torch.zeros((points.shape[0],3),dtype=torch.float32)
                bev_mapping_pv = torch.zeros((points.shape[0]), dtype=torch.int32)
                bev_mapping_vf = torch.zeros((voxel_generator_cfg.MAX_NUMBER_OF_BEV_VOXEL, 3), dtype=torch.int32)

                self.hv_features[j].dynamicVoxelBEVFaster(points, bev_local_coordinate, bev_mapping_pv, bev_mapping_vf)
                valid_bev_voxel_nums = self.hv_features[j].getValidBEVVoxelNums()
                # only use y x coordinate
                bev_mapping_vf = bev_mapping_vf[:valid_bev_voxel_nums, :][:,:2]
                example['voxel_local_coordinates'].append(bev_local_coordinate)
                example['bev_mapping_pvs'].append(bev_mapping_pv)
                example['bev_mapping_vfs'].append(bev_mapping_vf)
    ...
    def collate_batch(batch_list, _unused=False):
        ...
        for key, elems in example_merged.items():
            if key in ['voxels', 'num_points', 'voxel_centers', 'seg_labels', 'part_labels', 'bbox_reg_labels']:
            ...
            elif key in ['bev_coordinate', 'bev_local_coordinate', 'fv_local_coordinate', 'intensity', 'bev_points']:
                ...
                ret[key] = ret[key].long()
            elif key in ['bev_mapping_vfs']: #
                ret[key] = list()
                for i in range(len(elems[0])):
                    for idx, elem in enumerate(elems):
                        cur_elem = torch.cat((idx * torch.ones((elem[i].shape[0],1),dtype=torch.int32),elem[i]), dim=1)
                        if idx == 0:
                            ret[key].append(cur_elem)
                        else:
                            ret[key][-1] = torch.cat((ret[key][-1], cur_elem),dim=0)
                    ret[key][-1] = ret[key][-1].long()
            
            elif key in ['bev_mapping_pvs']:
                ret[key] = list()
                for i in range(len(elems[0])):
                    cur_voxel_num = 0
                    for idx, elem in enumerate(elems):
                        if idx == 0:
                            ret[key].append(elem[i])
                        else:
                            ret[key][-1] = torch.cat((ret[key][-1], elem[i] + cur_voxel_num), dim=0)
                        cur_voxel_num += example_merged['bev_mapping_vfs'][idx][i].shape[0]
                    
                    ret[key][-1] = ret[key][-1].long()
            elif key in ['voxel_local_coordinates']:
                ret[key] = list()
                for i in range(len(elems[0])):
                    for idx,elem in enumerate(elems):
                        if idx == 0:
                            ...
                            ret[key][-1] = torch.cat((ret[key][-1], elem[i]), dim=0)
            else:
