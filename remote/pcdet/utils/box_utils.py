...
def boxes3d_to_corners2d_lidar_torch(boxes3d, ry_flag):
    """
    NOTE!!! 
    ...
    x_corners = torch.cat([w / 2., -w / 2., -w / 2., w / 2.], dim=1)  # (N, 8)
    y_corners = torch.cat([-l / 2., -l / 2., l / 2., l / 2.], dim=1)  # (N, 8)

    temp_corners = torch.cat((
        x_corners.unsqueeze(dim=2), y_corners.unsqueeze(dim=2)
    ), dim=2)  # (N, 8, 2)
    # convert ry to countclockwise angle -> np.pi/2 - ry
    if ry_flag:
        ry = -np.pi * 0.5 - ry
    ...
    rotated_corners = torch.matmul(temp_corners, R)  # (N, 4, 2)
    x_corners, y_corners = rotated_corners[:, :, 0], rotated_corners[:, :, 1]
    x_loc, y_loc = boxes3d[:, 0], boxes3d[:, 1]

    x = x_loc.view(-1, 1) + x_corners.view(-1, 4)
    y = y_loc.view(-1, 1) + y_corners.view(-1, 4)
    z = z_loc.view(-1, 1) + z_corners.view(-1, 4)
    corners = torch.cat((x.view(-1, 4, 1), y.view(-1, 4, 1)), dim=2)
    corners = corners.reshape(-1,8)
    return corners

def boxes3d_to_corners2d_lidar(boxes3d, ry_flag):
    """
    :param boxes3d: (N, 7) [x, y, z, w, l, h, ry] in LiDAR coords, see the definition of ry in KITTI dataset
    ...
    y_corners = np.array([w / 2., -w / 2., -w / 2., w / 2.], dtype=np.float32).T
    x_corners = np.array([-l / 2., -l / 2., l / 2., l / 2.], dtype=np.float32).T
    ...
    temp_corners = np.concatenate((x_corners.reshape(-1, 4, 1), y_corners.reshape(-1, 4, 1)), axis=2)  # (N, 8, 3)
