...
class HVFeatureNetFinal(VoxelFeatureExtractor):
    ...
    def forward(self, input_dict):
        ...
        for i in range(self.input_scale_nums):
            ...
            AVFE_features.append(torch.cat((final_feature, point_voxel_feature), dim=1).contiguous())
        ...
        #     plt.imsave('/root/tmp/img_batch_{}_bev.jpg'.format(j),tt)

        # for i in range(len(AVFEO_features)):
        #     for j in range(batch_size):
        ...
        #         print(feature.max())
        #         print(feature.min())
        #         tt = np.ones(feature.shape)
        #         plt.imsave('/root/tmp/AVFEO_img_batch_{}_{}.jpg'.format(j, i),feature)
