...
class LinearWarmupLR(lr_sched._LRScheduler):
    def __init__(self, optimizer, start_ratio, iterations, last_epoch=-1):
    ...
    def get_lr(self):
        if self.last_epoch < self.max_iterations:
            return [((self.start_ratio + (1 - self.start_ratio) * self.last_epoch / self.max_iterations) * base_lr) for base_lr in self.base_lrs]
        else:
